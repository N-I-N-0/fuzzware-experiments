...
( for run_no in `seq 1 $experiment_repetition_count`; do
    ...
    for target in $target_list; do
        echo "fuzzware pipeline --runtime-config-name config_with_exits.yml -p $project_name $extra_emu_args $DIR/$target && fuzzware genstats -p $DIR/$target/$project_name $statistics_names"
    done
